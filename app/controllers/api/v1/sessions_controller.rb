class Api::V1::SessionsController < ApplicationController
  skip_before_action :authorize_request, only: [:create, :google_login]
  def create
  ...
  def google_login
    user = User.find_by(email: GetGoogleUserinfo.get_access_token(params[:authorization_code]).email)
    if user
      jwt = JwtToken.render_user_authorized_token user.jwt_payload
      render json: { token: "Bearer #{jwt}", user: UserSerializer.new(user) }
    else
